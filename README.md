...
`rig` sets up and runs an agentic coding workflow. The workflow has three
Roles: plan, implement, and review. Each Role uses a Harness — an agent CLI
such as Claude Code or Codex. `rig` drives these Harnesses from one
command-line tool.
...
This section gives the words to use when you talk about `rig`. Use these
words. Do not use other words for the same ideas.

**Harness**
An agent CLI that `rig` drives. Example: Claude Code, Codex.
...
**Skill set**
The set of skills stored in this repository. `rig init` installs the Skill
set into a project.
...
The installer checks the release checksum. It writes `rig` to `~/.local/bin`
...
You can also run `rig` without a binary:
...
```sh
# Scaffold the project. rig asks questions about Trackers, Roles, and skills.
rig init

# Implement issue #42. rig runs implement, then review, in a loop.
rig implement '#42'
```

`rig implement` stops when the reviewer Role returns an `approve` Verdict, or
...
Run `rig init` from the project root:

```sh
rig init
```

`rig init` asks these questions:
...
- Writes the project configuration to `.rig/config.toml`.
- Installs the Skill set into the project.
- Links a Claude Code integration, when Claude Code is a configured Harness.
- Adds `.rig/runs/` to `.gitignore`.

`rig init` shows every planned change and asks for confirmation before it
writes anything. Run `rig init` again to review or add optional skills; it
never overwrites a file that already holds your changes.

### 2. Configure the workflow

`rig` reads `.rig/config.toml`. A generated file looks like this:

```toml
# Generated by rig init.
[tracker]
issues = "github"
reviews = "local"
...
notifications. `rig` rejects a config file that has an unknown key, so remove
...
`rig review` runs the reviewer Role once, against the current working-tree
...
When the Review log is `github`, an issue reference is required; `rig`
...
A Harness can pause mid-run and print a `QUESTION`. When it does, `rig`
...
| `loop.max_iterations` | integer | Cap on implement/review iterations for `rig implement`. |
...
`opencode` is a recognized Harness value in the configuration, but `rig` does
not yet drive it; select `claude` or `codex` for `roles.implement.harness`
and `roles.review.harness` today.

## Run artifacts

Each `rig implement` run writes its artifacts under
`.rig/runs/<timestamp>-<issue-number>/`:

```text
.rig/runs/20260817T101500.000000000Z-42/
